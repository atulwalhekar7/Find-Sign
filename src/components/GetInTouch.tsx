...
const THEMES = {
  dark: {
    ...
    inputBg: "#1E1E1E",
    inputText: "#F9F9F9",
    inputBorder: "rgba(105, 228, 220, 0.2)",
...
export default function GetInTouch({ initialService = "", showService = false, hideInternalHeading = false }: NakraniContactProps) {
  ...
  const selectArrowColor = useMemo(() => encodeURIComponent(t.headingColor), [t.headingColor]);
...
.toggle-btn:hover {
  ...
  transform: translateY(-3px);
  box-shadow: 0 6px 12px rgba(105, 228, 220, 0.25);
}
...
.submit-btn:disabled {
  background: ${isDark ? "rgba(105, 228, 220, 0.1)" : "#ccc"};
  color: ${isDark ? "rgba(255, 255, 255, 0.3)" : "#666"};
  cursor: not-allowed;
  opacity: 0.7;
}
    ...
    >
      ...
      >
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 2 }}>
          ...
          <Typography tabIndex={0} sx={{ fontFamily: 'GT Super Display', fontSize: '32px', color: t.headingColor, fontWeight: 500 }}>
            Success!
          </Typography>
          <Typography tabIndex={0} sx={{ fontFamily: 'sohneBuch', fontSize: '18px', color: t.dialogText, fontWeight: 300, lineHeight: '28px' }}>
